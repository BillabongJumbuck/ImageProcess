...
public partial class MainWindowViewModel : ObservableObject
{
    ...
    [ObservableProperty] private ObservableCollection<ImageFile> imageFiles = new();

    [ObservableProperty] private ImageFile? selectedFile;

    [ObservableProperty] private ObservableCollection<string> processTypes;

    [ObservableProperty] private string? selectedProcessType;
                ...
                var thread = new Thread(async () =>
                {
                    try
                    {
                        ...
                        WindowsMessage.SendProgressMessage(mainWindowHandle, index,
                            WindowsMessage.ProcessStatus.Processing);

                        bool success = false;
                        try
                        {
                            success =
                                ProcessTypeSuffixes.GetValueOrDefault(SelectedProcessType ?? "", "_processed") switch
                                {
                                    "_gray" => await Utility.ImageProcess.ToGrayScale(file.FilePath, outputPath,
                                        cts.Token),
                                    "_scale200" => await Utility.ImageProcess.Scale200(file.FilePath, outputPath,
                                        cts.Token),
                                    "_scale50" => await Utility.ImageProcess.Scale50(file.FilePath, outputPath,
                                        cts.Token),
                                    "_rotate_cw" => await Utility.ImageProcess.RotateClockwise90(file.FilePath,
                                        outputPath, cts.Token),
                                    "_rotate_ccw" => await Utility.ImageProcess.RotateCounterClockwise90(file.FilePath,
                                        outputPath, cts.Token),
                                    "_edge" => await Utility.ImageProcess.EdgeDetection(file.FilePath, outputPath,
                                        cts.Token),
                                    "_binary" => await Utility.ImageProcess.Threshold(file.FilePath, outputPath,
                                        cts.Token),
                        ...
                        {
                            WindowsMessage.SendProgressMessage(mainWindowHandle, index,
                                WindowsMessage.ProcessStatus.Cancelled);
                        }
                    }
                    finally
                    {
